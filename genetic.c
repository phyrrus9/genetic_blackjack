...
#define DEATH 1 /* lower DEATH of solns die */
#define MAX_MUTATIONS 7 /* maximum number of mutations per step */
#define GENETIC_PLAYERS 5
...
struct genetic
{
	...
	action_t st1[10][13][13]; //strategy1 (initial deal)
	action_t st2[10][20]; //strategy2 (everything else)
...
void pool_init(struct pool *s)
{
	...
	for (i = 0; i < GENETIC_PLAYERS; i++) pn[i] = 0;
	pc = 0;
...
struct genetic mutate1(const struct genetic s, card_t c1, card_t c2)
{
	...
	for (i = 0; i < mutations; i++)
	{
		...
		if (mutator == kSPLIT)
			if (cansplit(c1, c2)
				ret->st1[upcard][first][second] = kSPLIT;
		ret->st1[upcard][first][second] = mutator;
	}
	return ret;
}
